...
<head>
	<meta charset="UTF-8">
	<title>教师列表</title>
	<style  type="text/css">@import url("<c:url value="/easyui/themes/default/easyui.css"/>");</style>
	<style type="text/css">@import url("<c:url value="/easyui/themes/icon.css"/>");</style>
	<style type="text/css">@import url("<c:url value="/easyui/css/demo.css"/>");</style>
	...
	$(function() {	
	    ...
	    function preLoadClazz(){
	  		$("#clazzList").combobox({
		  		...
		  		editable: false, //不可编辑
		  		method: "post",
		  		url: "<%=path%>/clazz/clazzList?from=combox",
	  ...
	  //下拉框通用属性
	  	...
	  	$("#edit_clazzList").combobox({
	  		url: "<%=path %>/clazz/clazzList?t="+new Date().getTime()+"&from=combox",
			onLoadSuccess: function(){
				//默认选择第一条数据
				var data = $(this).combobox("getData");
				$(this).combobox("setValue", data[0].id);
...
<body>
	...
	<div id="toolbar">
		...
		<div style="float: left;margin-top:4px;" class="datagrid-btn-separator" >&nbsp;&nbsp;姓名：<input id="search_teacher_name" class="easyui-textbox" name="search_student_name" /></div>
